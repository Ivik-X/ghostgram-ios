...
private enum SGBoolSetting: String {
    ...
    case miscBypassCopyProtection
    case miscDisableViewOnceAutoDelete
    case miscBlockAds
    case ghostModeEnabled
    case ghostModeHideOnline
    case ghostModeHideReadReceipts
    case ghostModeHideTyping
    case ghostModeHideStoryViews
    case ghostModeForceOffline
    case antiDeleteEnabled
    case antiDeleteArchiveMedia
    case antiDeleteKeepLocal
    case antiDeleteDisplayEdited
...
private enum SGDisclosureLink: String {
    ...
    case voiceClone
    case pluginList
    case antiDeleteExclusions
    case ghostModeExclusions
}
...
private func SGControllerEntries(presentationData: PresentationData, callListSettings: CallListSettings, experimentalUISettings: ExperimentalUISettings, appConfiguration: AppConfiguration, nameColors: PeerNameColors, state: SGSettingsControllerState) -> [SGControllerEntry] {
    ...
    // MARK: Ghost Mode
    entries.append(.header(id: id.count, section: .ghostMode, text: "РЕЖИМ ПРИЗРАКА (GHOST MODE)", badge: nil))
    entries.append(.toggle(id: id.count, section: .ghostMode, settingName: .ghostModeEnabled, value: GhostModeManager.shared.isEnabled, text: "Режим призрака", enabled: true))
    entries.append(.toggle(id: id.count, section: .ghostMode, settingName: .ghostModeHideOnline, value: GhostModeManager.shared.hideOnlineStatus, text: "Не показывать «В сети»", enabled: GhostModeManager.shared.isEnabled))
    entries.append(.toggle(id: id.count, section: .ghostMode, settingName: .ghostModeHideReadReceipts, value: GhostModeManager.shared.hideReadReceipts, text: "Скрывать прочтение сообщений", enabled: GhostModeManager.shared.isEnabled))
    entries.append(.toggle(id: id.count, section: .ghostMode, settingName: .ghostModeHideTyping, value: GhostModeManager.shared.hideTypingIndicator, text: "Скрывать статус «Печатает...»", enabled: GhostModeManager.shared.isEnabled))
    entries.append(.toggle(id: id.count, section: .ghostMode, settingName: .ghostModeHideStoryViews, value: GhostModeManager.shared.hideStoryViews, text: "Скрывать просмотр историй", enabled: GhostModeManager.shared.isEnabled))
    entries.append(.toggle(id: id.count, section: .ghostMode, settingName: .ghostModeForceOffline, value: GhostModeManager.shared.forceOffline, text: "Принудительный офлайн", enabled: GhostModeManager.shared.isEnabled))
    entries.append(.notice(id: id.count, section: .ghostMode, text: "Ghost Mode позволяет читать чаты и просматривать истории полностью анонимно."))
    
    // MARK: Anti-Delete
    entries.append(.header(id: id.count, section: .antiDelete, text: "УДАЛЕННЫЕ СООБЩЕНИЯ (ANTI-DELETE)", badge: nil))
    entries.append(.toggle(id: id.count, section: .antiDelete, settingName: .antiDeleteEnabled, value: AntiDeleteManager.shared.enabled, text: "Сохранять удаленные сообщения", enabled: true))
    entries.append(.toggle(id: id.count, section: .antiDelete, settingName: .antiDeleteArchiveMedia, value: AntiDeleteManager.shared.archiveMedia, text: "Сохранять фото и видео", enabled: AntiDeleteManager.shared.enabled))
    entries.append(.toggle(id: id.count, section: .antiDelete, settingName: .antiDeleteKeepLocal, value: AntiDeleteManager.shared.keepLocallyWhenDeletingForEveryone, text: "Сохранять при удалении для всех", enabled: AntiDeleteManager.shared.enabled))
    entries.append(.toggle(id: id.count, section: .antiDelete, settingName: .antiDeleteDisplayEdited, value: AntiDeleteManager.shared.displayEditedMessages, text: "История изменений сообщений", enabled: AntiDeleteManager.shared.enabled))
    entries.append(.notice(id: id.count, section: .antiDelete, text: "Удаленные собеседником сообщения сохраняются в чате."))
    
    // MARK: Local Premium
    entries.append(.header(id: id.count, section: .localPremium, text: "ЛОКАЛЬНЫЙ TELEGRAM PREMIUM", badge: nil))
    entries.append(.toggle(id: id.count, section: .localPremium, settingName: .localPremiumEnabled, value: LocalPremiumManager.shared.isEnabled, text: "Включить Telegram Premium локально", enabled: true))
    entries.append(.notice(id: id.count, section: .localPremium, text: "Разблокирует все функции Telegram Premium на клиенте (значок, эмодзи-статусы, ускоренная загрузка)."))
    
    // MARK: Misc Features
    entries.append(.header(id: id.count, section: .miscFeatures, text: "ДОПОЛНИТЕЛЬНЫЕ ФУНКЦИИ", badge: nil))
    entries.append(.toggle(id: id.count, section: .miscFeatures, settingName: .miscBypassCopyProtection, value: MiscSettingsManager.shared.bypassCopyProtection, text: "Обход защиты от копирования", enabled: true))
    entries.append(.toggle(id: id.count, section: .miscFeatures, settingName: .miscDisableViewOnceAutoDelete, value: MiscSettingsManager.shared.disableViewOnceAutoDelete, text: "Сохранять одноразовые фото/видео", enabled: true))
    entries.append(.toggle(id: id.count, section: .miscFeatures, settingName: .miscBlockAds, value: MiscSettingsManager.shared.blockAds, text: "Блокировать официальную рекламу", enabled: true))
    entries.append(.notice(id: id.count, section: .miscFeatures, text: "Позволяет сохранять защищенные медиафайлы и копировать текст из закрытых каналов."))
    
    // MARK: Ghostgram Subsystems
    entries.append(.header(id: id.count, section: .miscFeatures, text: "GHOSTGRAM", badge: "PRO"))
    entries.append(.disclosure(id: id.count, section: .miscFeatures, link: .deviceSpoof, text: "Подмена устройства"))
    entries.append(.disclosure(id: id.count, section: .miscFeatures, link: .fontCustomization, text: "Кастомизация UI и шрифтов"))
    entries.append(.disclosure(id: id.count, section: .miscFeatures, link: .fakeGifts, text: "Фейковые подарки"))
    entries.append(.disclosure(id: id.count, section: .miscFeatures, link: .messageBookmarks, text: "Закладки сообщений"))
    entries.append(.disclosure(id: id.count, section: .miscFeatures, link: .videoFeed, text: "Видеолента"))
    entries.append(.disclosure(id: id.count, section: .miscFeatures, link: .videoMasks, text: "Видеомаски для кружочков"))
    entries.append(.disclosure(id: id.count, section: .miscFeatures, link: .voiceClone, text: "Клонирование голоса"))
    entries.append(.disclosure(id: id.count, section: .miscFeatures, link: .pluginList, text: "Плагины и скрипты"))
    entries.append(.disclosure(id: id.count, section: .miscFeatures, link: .antiDeleteExclusions, text: "Исключения Анти-Удаления"))
    entries.append(.disclosure(id: id.count, section: .miscFeatures, link: .ghostModeExclusions, text: "Исключения Режима Призрака"))
    entries.append(.notice(id: id.count, section: .miscFeatures, text: "Полный набор продвинутых возможностей Ghostgram."))
...
//    }
    ...
    let arguments = SGItemListArguments<SGBoolSetting, SGSliderSetting, SGOneFromManySetting, SGDisclosureLink, AnyHashable>(
        ...
        case .miscBypassCopyProtection:
            ...
            simplePromise.set(true)
        case .miscDisableViewOnceAutoDelete:
            MiscSettingsManager.shared.disableViewOnceAutoDelete = value
            simplePromise.set(true)
        case .miscBlockAds:
        ...
        case .ghostModeEnabled:
            ...
            simplePromise.set(true)
        case .ghostModeHideOnline:
            GhostModeManager.shared.hideOnlineStatus = value
            simplePromise.set(true)
        case .ghostModeHideReadReceipts:
            GhostModeManager.shared.hideReadReceipts = value
            simplePromise.set(true)
        case .ghostModeHideTyping:
            GhostModeManager.shared.hideTypingIndicator = value
            simplePromise.set(true)
        case .ghostModeHideStoryViews:
            GhostModeManager.shared.hideStoryViews = value
            simplePromise.set(true)
        case .ghostModeForceOffline:
            GhostModeManager.shared.forceOffline = value
            simplePromise.set(true)
        case .antiDeleteEnabled:
            AntiDeleteManager.shared.enabled = value
            simplePromise.set(true)
        case .antiDeleteArchiveMedia:
            AntiDeleteManager.shared.archiveMedia = value
            simplePromise.set(true)
        case .antiDeleteKeepLocal:
            AntiDeleteManager.shared.keepLocallyWhenDeletingForEveryone = value
            simplePromise.set(true)
        case .antiDeleteDisplayEdited:
...
//            }
        ...
        switch (link) {
            ...
            case .fontCustomization:
                pushControllerImpl?(fontCustomizationController(context: context))
            case .fakeGifts:
                pushControllerImpl?(fakeGiftsController(context: context))
            case .messageBookmarks:
                pushControllerImpl?(messageBookmarksController(context: context))
            case .videoFeed:
                pushControllerImpl?(videoFeedScreenController(context: context))
            case .videoMasks:
                pushControllerImpl?(videoMasksController(context: context))
            case .voiceClone:
                pushControllerImpl?(voiceCloneLibraryController(context: context))
            case .pluginList:
                pushControllerImpl?(pluginListController(context: context))
            case .antiDeleteExclusions:
                pushControllerImpl?(antiDeleteFolderExclusionsController(context: context))
            case .ghostModeExclusions:
                pushControllerImpl?(ghostModeFolderExclusionsController(context: context))
        }
