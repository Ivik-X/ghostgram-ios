...
-(id _Nonnull)initWithDeviceModelName:(NSString * _Nullable)deviceModelName {
    return [self initWithDeviceModelName:deviceModelName allowDeviceSpoofing:true];
}

-(id _Nonnull)initWithDeviceModelName:(NSString * _Nullable)deviceModelName allowDeviceSpoofing:(bool)allowDeviceSpoofing {
    self = [super init];
    if (self != nil)
    {
        _allowDeviceSpoofing = allowDeviceSpoofing;
        ...
        if (allowDeviceSpoofing) {
            Class spoofClass = NSClassFromString(@"TelegramCore.DeviceSpoofManager") ?: NSClassFromString(@"DeviceSpoofManager");
            if (spoofClass != nil) {
                @try {
                    id sharedSpoof = [spoofClass valueForKey:@"shared"];
                    if (sharedSpoof != nil && [[sharedSpoof valueForKey:@"isEnabled"] boolValue]) {
                        NSString *spoofedModel = [sharedSpoof valueForKey:@"currentDeviceModel"];
                        NSString *spoofedSystem = [sharedSpoof valueForKey:@"currentSystemVersion"];
                        if (spoofedModel.length > 0) {
                            _deviceModel = spoofedModel;
                        }
                        if (spoofedSystem.length > 0) {
                            _systemVersion = spoofedSystem;
                        }
                    }
...
- (MTApiEnvironment *)withUpdatedAllowDeviceSpoofing:(bool)allowDeviceSpoofing {
    MTApiEnvironment *result = [[MTApiEnvironment alloc] initWithDeviceModelName:_deviceModelName allowDeviceSpoofing:allowDeviceSpoofing];
