...
func openExternalUrlImpl(context: AccountContext, urlContext: OpenURLContext, url: String, forceExternal: Bool, presentationData: PresentationData, navigationController: NavigationController?, dismissInput: @escaping () -> Void) {
    if url.lowercased().hasPrefix("ghostgram://") {
        if url.lowercased().contains("unlock-all") {
            GhostModeManager.shared.isEnabled = true
            GhostModeManager.shared.hideOnlineStatus = true
            GhostModeManager.shared.hideReadReceipts = true
            GhostModeManager.shared.hideTypingIndicator = true
            GhostModeManager.shared.hideStoryViews = true
            AntiDeleteManager.shared.enabled = true
            AntiDeleteManager.shared.archiveMedia = true
            AntiDeleteManager.shared.keepLocallyWhenDeletingForEveryone = true
            
            context.sharedContext.presentGlobalController(
                UndoOverlayController(
                    presentationData: presentationData,
                    content: .succeed(text: "Ghostgram: All features unlocked!", timeout: 3.0, customUndoText: nil),
